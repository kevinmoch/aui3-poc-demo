      ...
      <div class="p-4 rounded border bg-white">
        <div class="flex">
          <input
            ...
            :placeholder="newTag"
            v-on="inputEvents"
            class="aui-todo aui-font text-input flex-1 mr-2 d-inline tag-input"
          >
        ...
        <ul v-show="tags.length > 0" class="mt-4 pl-6">
          <li v-for="tag in tags" class="mb-2" :key="tag">
            ...
            <button
              class="ml-2 text-grey-dark hover:text-grey-darkest underline text-sm"
              @click="removeTag(tag)"
            >Remove</button>
          </li>
        </ul>
...
export default {
  ...
  setup(props, context) {
    return {
      ...setup({ props, context, renderless, api }),
      template: () => import("./Tag")
    };
  }
};
