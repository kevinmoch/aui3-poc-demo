      ...
      <div className="p-4 rounded border bg-white">
        <div className="flex">
          <input
            ...
            onChange={input}
            onKeyDown={keydown}
            className="aui-todo aui-font text-input flex-1 mr-2 d-inline tag-input"
          />
          ...
          {tags.map(tag => {
            return (
              <li key={tag} className="mb-2">
                <span className="aui-tag aui-font text-capitalize font-weight-bold">
                  {tag}
                </span>
                <button
                  className="ml-2 text-grey-dark hover:text-grey-darkest underline text-sm"
                  onClick={() => {
                    removeTag(tag);
                  }}
                >
